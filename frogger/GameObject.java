...
public abstract class GameObject {
  ...
  /**
   * Returns a random number between the min param and the max param only if the
   * auto param is true
   * 
   * @param min  The minimum desired random value.
   * @param max  The maximum desired value.
   * @param auto Determines whether or not to return an actual lane position or an
   *             actual random value between the min and max values.
   * @return A value between min and max or a lane position based on the min and
   *         max.
   */
