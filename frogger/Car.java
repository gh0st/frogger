...
public class Car extends Vehicles implements IDrawable, ICollider, ISelectable {
   ...
   * 
   * @param gw the gameworld which will hold the car so that it has access to the
   *           global vin.
   */
  ...
  public void draw(Graphics2D g2d) {
    /*
     * removing this in favor of affinetransforms g2d.setColor(getColor());
     * g2d.drawRect(getX()-(getWidth()/2), getY()-(getHeight()/2), getWidth(),
     * getHeight());
     */
  ...
  public String toString() {
    return "Car: " + getLocation() + " " + getColor() + " speed=" + getSpeed() + " dir=" + getDirection()
        + " passengerCount=" + getPassengerCount() + " vin=" + getVIN();
  }
  ...
  public boolean contains(Point p) {
    ...
    if ((px >= xLoc - (getWidth() / 2)) && (px <= xLoc + (getWidth() / 2)) && (py >= yLoc - (getHeight() / 2))
        && (py <= yLoc + (getHeight() / 2))) {
      return true;
    } else {
      return false;
